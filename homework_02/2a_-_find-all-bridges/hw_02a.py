...
#    sys.stdout.write(BColors.RESET)
  ...
  # test for multiple line from same node to same node
  for index in range(len(list_of_start_nodes)):
    for i in range(len(list_of_start_nodes)):
      if ((list_of_start_nodes[index] == list_of_start_nodes[i]) & (index != i)):
        if (list_of_end_nodes[index] == list_of_end_nodes[i]):
          sys.stdout.write(BColors.RED)
          print 'Stav site ERROR'
          sys.stdout.write(BColors.RESET)
          sys.exit()
  ...
  # test for loop (smycka) in graph
  for index in range(len(list_of_start_nodes)):
    if (list_of_start_nodes[index] == list_of_end_nodes[index]):
      sys.stdout.write(BColors.RED)
      print 'Stav site ERROR'
      sys.stdout.write(BColors.RESET)
      sys.exit()
